...
export function BandVideoEditor({ bandId, initialUrls, readOnly = false }: Props) {
  const [urls, setUrls] = useState<string[]>(initialUrls)
  const [input, setInput] = useState('')
  const supabase = createClient()

  async function save(next: string[]) {
    await supabase.from('bands').update({ video_urls: next } as any).eq('id', bandId)
  }
  ...
  if (readOnly) {
    ...
    return (
      <div>
        <h3 className="label">Videolar</h3>
  ...
  return (
    <div>
      <h3 className="label">Videolar <span className="font-normal text-text-muted normal-case">(YouTube / Vimeo)</span></h3>
