    ...
    <div>
            ...
            <label className="flex items-center gap-2 cursor-pointer select-none ml-auto">
              <div
                ...
                className={cn(
                  ...
                  hiringOnly ? 'bg-yellow-400' : 'bg-[rgba(228,224,216,0.15)]'
                )}
              >
                <span className={cn(
                  'absolute top-0.5 w-4 h-4 rounded-full bg-white transition-transform',
                  hiringOnly ? 'translate-x-4' : 'translate-x-0.5'
                )} />
              </div>
              <span className="text-xs text-text-muted whitespace-nowrap">Üye arayanlar</span>
            </label>
          )}
        </div>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16 text-text-muted text-sm">
          ...
          <p>Bu kriterlere uyan grup bulunamadı.</p>
        </div>
      ) : (
          ...
          {filtered.map((band) => {
            ...
            return (
              <Link
                ...
                href={`/bands/${band.id}`}
                className={cn(
                  'card p-4 hover:border-accent/30 transition-colors flex gap-4',
                  isHiring && 'border-yellow-400/20'
                )}
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-3 text-xs text-text-muted mt-0.5">
                    {band.city && (
                      <span className="flex items-center gap-1"><MapPin size={10} />{band.city}</span>
                    )}
                    <span className="flex items-center gap-1"><Users size={10} />{memberCount} üye</span>
                  </div>
                  ...
                  {isHiring && (
                    <div className="flex flex-wrap items-center gap-1 mt-2">
                      ...
                      {band.looking_for.map((item) => (
                        <span key={item} className="text-[10px] px-1.5 py-0.5 rounded bg-yellow-400/10 text-yellow-400 border border-yellow-400/20">
                          {item}
                        </span>
                      ))}
                    </div>
