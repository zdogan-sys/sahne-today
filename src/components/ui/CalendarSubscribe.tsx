  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        ...
        {isEn ? 'Subscribe to Calendar' : 'Takvime Abone Ol'}
      ...
      {open && (
        <div className="absolute left-0 top-full mt-2 w-64 bg-surface border border-[rgba(228,224,216,0.12)] rounded-xl shadow-2xl z-50 overflow-hidden">
          <p className="px-4 pt-3 pb-1 text-[10px] text-text-muted uppercase tracking-wider">{isEn ? 'Automatic Sync' : 'Otomatik Senkronizasyon'}</p>
          ...
          <div className="border-t border-[rgba(228,224,216,0.08)] px-4 pt-2 pb-3">
            <p className="text-[10px] text-text-muted mb-2">{isEn ? 'Manual download' : 'Manuel indirme'}</p>
            <a
              href={feedUrl}
              ...
              onClick={() => setOpen(false)}
            >
              {isEn ? 'Download .ics file →' : '.ics dosyası indir →'}
            </a>
          </div>
        </div>
      )}
