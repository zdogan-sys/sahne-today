...
import { MUSIC_GENRES, STAGE_GENRES, INSTRUMENT_OPTIONS } from '@/lib/constants'

const ADMIN_EMAIL = 'z_dogan@hotmail.com'

export type ListConfigKey = 'music_genres' | 'stage_genres' | 'instruments'
...
export async function getListConfigs(): Promise<Record<ListConfigKey, string[]>> {
  const { data } = await adminClient()
    .from('site_settings')
    .select('key, value')
    .in('key', ['music_genres', 'stage_genres', 'instruments'])
