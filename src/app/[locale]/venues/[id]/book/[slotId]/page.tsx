...
export default function BookVenueSlotPage() {
  ...
  useEffect(() => {
    async function load() {
      const { data: slotData } = await supabase
        .from('teaching_slots')
        .select('*, venues(id, name, city, photo_url)')
        .eq('id', slotId)
        .single()

      setSlot(slotData)
      setVenue((slotData as any)?.venues)
  ...
  if (!slot) return <div className="max-w-lg mx-auto px-4 py-12 text-center"><p className="text-text-muted">Slot bulunamadı.</p><Link href={`/venues/${id}`} className="text-accent mt-2 block">Geri dön →</Link></div>

  const availableDates = slot.slot_date
    ? [slot.slot_date]
    : getNextDates(slot.day_of_week, slot.recurrence ?? 'weekly')
  ...
  if (booked) return (
    <div className="max-w-lg mx-auto px-4 py-16 text-center">
      ...
      <p className="text-text-muted text-sm">Mekan onayladığında bildirim alacaksınız.</p>
      <Link href={`/venues/${id}`} className="text-accent mt-4 block hover:underline">Mekan profiline dön →</Link>
    </div>
  )
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-6">
      ...
      <div className="card p-4 mb-6 flex items-center justify-between">
        <div>
          <p className="text-text-primary font-semibold">{slot.instructor_name ?? slot.instrument}</p>
          <p className="text-accent text-xs mt-0.5">{slot.instrument}</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button type="submit" disabled={submitting} className="btn-accent w-full py-3 text-sm disabled:opacity-50 flex items-center justify-center gap-2">
          {submitting ? <><Loader2 size={15} className="animate-spin" /> İşleniyor...</> : slot.payment_enabled ? 'Ödemeye Geç' : 'Rezervasyon Yap'}
        </button>
      </form>
    </div>
  )
